...
<body>
	<jsp:include page="headernhanvien.jsp"></jsp:include>
	<div class="container">
		 <table class="table">
			<%NguoiDungBO ndbo = new NguoiDungBO();
			ArrayList<DonHangBEAN> dsdonhang = (ArrayList<DonHangBEAN>)request.getAttribute("dsxacnhan");
	  		...
	  		<tbody>	
				<%//Hiển thị 
					...
					for(int i = 0; i< soLuongHoaDon; i++){
						NguoiDungBEAN nguoidung = ndbo.selectTenNguoiDungByID(dsdonhang.get(i).getIdNguoiDung());
						%>
				  		<tr>
					      	<td><%=dsdonhang.get(i).getIdDonHang()%></td>
					      	<td><%=nguoidung.getHoTen()%></td>
				      		<td><%=dsdonhang.get(i).getNgayDat()%></td>	
			      			<td><a href="xacnhan?action=showctdh&iddonhang=<%=dsdonhang.get(i).getIdDonHang()%>"><button type="button" class="btn btn-success btn-sm">Chi tiết đơn hàng</button></a></td>
					     </tr>   
					<%}%>
	  		</tbody>
			<%}else if(dsctdh!=null){
		     ...
		     <% }%>
  			...
  			<tfoot>
			    <tr>
	      		...
	      		<td>
	      			<a href="xacnhan?action=xacnhan&iddonhang=<%=iddonhang%>">									      		
	      				<button type="button" class="btn btn-success form-control btn-sm my-auto h-auto">Xác nhận</button>		  		
	      			</a>
	      		</td>
	     	</tfoot>
	     	<%}else{%>	
