...
/**
 * Servlet implementation class AllDonHangCONTROLLER
 */
@WebServlet("/donhang")
public class AllDonHangCONTROLLER extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AllDonHangCONTROLLER() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String url = "allhoadonadmin.jsp";
		DonHangBO dhbo = new DonHangBO();
		ChiTietDonHangBO ctdhbo = new ChiTietDonHangBO();
		ArrayList<DonHangBEAN> dsalldh = new ArrayList<DonHangBEAN>();
				...
				if(!iddonhang.equals("null")) {
					...
					if (action.equals("xacnhan")) {
						try {
							...
							dsalldh = dhbo.selectAllDHChoXacNhan();
						}catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						request.setAttribute("dsxacnhan", dsalldh);
					}
				}
			}else {				
				try {
					dsalldh = dhbo.selectAllDHDaXacNhan();
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				request.setAttribute("dsalldhxacnhan", dsalldh);
//			}
		}
